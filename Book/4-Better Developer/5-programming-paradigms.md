...
Along with design patterns and algorithms, there is a thing called "Programming Paradigms" which is a style of computer programming. Much like a design pattern but it may or may not be tightly coupled to the language itself. Some languages support multiple paradigms.
...
When the program gets loaded, each of these lines gets run. You have the basic loops, control flows and everything else. You can even use functions. The point of procedural programming is to break down a task into variables and data structures as well as "subroutines" which can also be called a "function".
...
Functional programming is VERY hot right now, so hot in fact that PHP, C\#, and Java have added features to their languages in order to help use this paradigm in their respective domains. The most known functional language is Haskell; however, Javascript developers started to push the language far into those regions as well.
...
Some of the best examples of functional programming in JS is using the following "helpers": `Array.filter`, `Array.map`, and `Array.reduce`. Like so:

```js
var list = [{ name: 'John', status: 'ACTIVE'}, { name: 'Joe', status: 'INACTIVE'}, { name: 'Gene', status: 'ACTIVE'}];
...
console.log(newList); //result would be: ['John', 'Gene']
```
...
In this example, Javascript tells the browser to add an event listener (a function to be run) for a `click` event. When a user clicks on that element, the following function gets run. The `e` stands for event and information about it. By doing `preventDefault()`, we're preventing the browser's default event function to run (such as when clicking a link element, we're preventing the browser from redirecting to that link url).
